...
Reference<S3BlobStoreEndpoint> S3BlobStoreEndpoint::fromString(const std::string& url,
	...
	try {
		...
		if (isGCS) {
			throw std::string(
			    "GCS parameter should be handled by GCSBlobStore");
		}
...
std::string S3BlobStoreEndpoint::canonicalizeURI(const std::string& resource, std::vector<std::string>& queryParameters) {
