...
void parseProxy(Optional<std::string> const& proxy, Optional<std::string>& proxyHost, Optional<std::string>& proxyPort) {
	if (proxy.present()) {
...
void parseQueryParams(StringRef& resource, BlobKnobs& knobs, HTTP::Headers& extraHeaders, std::string& region, bool& isGCS, IBlobStoreEndpoint::ParametersT* ignored_parameters) {
		while (1) {
			StringRef name = resource.eat("=");
			if (name.size() == 0)
				break;
			StringRef value = resource.eat("&");
...
ACTOR Future<IBlobStoreEndpoint::ReusableConnection> connect_impl(Reference<IBlobStoreEndpoint> b,
                                                                   bool* reusingConn) {
