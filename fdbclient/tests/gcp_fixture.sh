...
#  if ! readarray -t configs < <(gcp_setup "${TEST_SCRATCH_DIR}"); then
...
# Set up GCS access.
# $1 gcp_dir (This is what is returned when you call create_gcp_dir
# so call it first).
...
function gcp_setup {
  ...
  if ! blob_credentials_file=$(write_blob_credentials "${GCS_APPLICATION_TOKEN}" "${host}" "${local_gcp_dir}"); then
